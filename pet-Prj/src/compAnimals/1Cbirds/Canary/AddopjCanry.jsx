...
export default function AddopjCanry({ setAddTodos, addTodos }) {
  ...
  function addTodo() {
    ...
    reader.onloadend = () => {
      ...
      const updateTodos = [...addTodos, newTodos];

      setAddTodos(updateTodos);
      localStorage.setItem("todoCanry", JSON.stringify(updateTodos));
    };
  ...
  useEffect(() => {
    const storageTodos = JSON.parse(localStorage.getItem("todoCanry")) ?? [];
    setAddTodos(storageTodos);
  }, []);
  ...
  return (
    <>
      ...
      <div className={`formilar-addinCats ${addActv} `}>
        <div className="formilers-canary d-flex w-50 align-items-center justify-content-center">
